...
input_path = './data/original_test'
output_path = './data/augmented_test'
...
def save_image(imgArr, output_path):
    ...
    for imgFile in imgArr:
        ...
        #翻转
        # path = os.path.join(output_path, '%s_%d.jpg' % ('augmented_flip90', i))

        # 调节亮度和对比度
        path = os.path.join(output_path, '%s_%d.jpg' % ('augmented_bri_0.3', i))

        # 随机亮度和对比度
        # path = os.path.join(output_path, '%s_%d.jpg' % ('augmented_rand_bri_0.5_contra_4', i))

        #随机色调
        # path = os.path.join(output_path, '%s_%d.jpg' % ('augmented_hue_0.3', i))

        #随机饱和度
        # path = os.path.join(output_path, '%s_%d.jpg' % ('augmented_satu_1.6', i))
...
def data_augmentation_tf(img_dir_path):
    ...
    for i in range(count):
        with tf.Session() as sess:
            ...
            """图像亮度[-1, 1]"""
            img_data = tf.image.adjust_brightness(img_data, delta=0.3)
            """图像对比度"""
            # img_data = tf.image.adjust_contrast(img_data, contrast_factor=4)
            """随机图像亮度 brightness"""
            ...
            """饱和度调整"""
            # img_data = tf.image.adjust_saturation(img_data, 1.6)
